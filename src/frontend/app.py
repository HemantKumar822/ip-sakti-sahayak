        ...
        st.markdown(
            ...
            <div class="callout-error">
                ...
                <div class="callout-error-body">
                    Cannot connect to Backend API. Please ensure the API server (<code>./run_api.sh</code>) is running and try again.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        if st.button("Try another question →", key="btn_retry_offline"):
            st.rerun()
    else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status == "abstained":
                    ...
                    if st.button("Try another question →", key="btn_retry_abstain"):
                        st.rerun()
                else:
                ...
                st.markdown(
                    ...
                    <div class="callout-error">
                        ...
                        <div class="callout-error-body">
                            Please wait a moment and try again. (Status: {response.status_code})
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                if st.button("Try another question →", key="btn_retry_err_status"):
                    st.rerun()
        except requests.RequestException:
            loading_placeholder.empty()
            st.markdown(
            ...
            if st.button("Try another question →", key="btn_retry_err_req"):
                st.rerun()
