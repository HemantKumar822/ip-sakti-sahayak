        ...
        with (
            ...
            response = asyncio.run(
                orchestrator.run_pipeline(
                    ...
                    session_id=f"test-{query_item['id']}-{int(time.time())}"
                )
            )
...
def test_consecutive_queries_performance_under_10_seconds():
    ...
    with (
        ...
        for idx, query in enumerate(queries, 1):
            ...
            resp = asyncio.run(
                orchestrator.run_pipeline(
                    query_text=query, session_id=f"perf-session-{idx}-{int(time.time())}"
                )
            )
            duration = time.perf_counter() - start
            assert (
    ...
    with (
        ...
        response = client.post(
            ...
            json={
                ...
                "session_id": f"test-error-session-{int(time.time())}",
            },
        )
